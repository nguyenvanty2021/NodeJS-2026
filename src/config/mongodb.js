...
const mongoClientInstance = new MongoClient(env.MONGODB_URI, {
  ...
  serverApi: {
    // Chỉ định phiên bản API sử dụng (hiện tại chỉ có v1)
    // Đảm bảo code hoạt động ổn định kể cả khi MongoDB nâng cấp server
    version: ServerApiVersion.v1,
    // Nếu true: MongoDB sẽ báo lỗi khi dùng command/feature không thuộc phiên bản API đã chọn
    // Giúp đảm bảo code chỉ dùng các tính năng ổn định
    strict: true,
    // Nếu true: MongoDB sẽ báo lỗi khi dùng command/feature đã bị deprecated
    // Giúp phát hiện sớm những API sắp bị loại bỏ
